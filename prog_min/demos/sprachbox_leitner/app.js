...
const els = {
  ...
  pillMeta: $("pillMeta"),
  pillBox: $("pillBox"),
  frontLangLbl: document.querySelector(".cardFront .lang"),
  backLangLbl: document.querySelector(".cardBack .lang"),
  front: $("txtFront"),
  back: $("txtBack"),
  mnemo: $("txtMnemo"),
  flip: $("btnFlip"),
...
setTxt("btnLogout", "Abmelden / Logout", "Aktives Profil abmelden");
...
const LEGACY_MIGRATION_FLAG = "LL:migrated:1";
const PROFILE_STATE_SCHEMA = 1;
...
const EXTRA_DATA_URLS = [
  ...
  "./data/song_ab70_de-ru.json",
];

const TOPIC_LABELS = {
  song_ab70_de_ru: "🎵 Song: Ab 70 blühe ich auf (DE→RU)",
};
...
const DEFAULT_PROFILE_STATE = JSON.parse(JSON.stringify({
  prog: state.prog,
  mnemoMap: state.mnemoMap,
  time: state.time,
  settings: state.settings,
  ui: {
    mode: state.ui.mode,
    uiMode: state.ui.uiMode,
  },
  daily: state.daily,
  boxFocus: state.boxFocus,
  deckOverrides: state.deckOverrides,
  glossary: {
    items: state.glossary.items,
    ui: state.glossary.ui,
  },
  lookup: {
    mini: state.lookup.mini,
    phraseMap: state.lookup.phraseMap,
  },
}));

let activeProfileId = null;
let profileBootstrapDone = false;
let scheduleProfileSave = () => {};
...
function getProfilePersistence() {
  return window.LLPersistence && typeof window.LLPersistence === "object"
    ? window.LLPersistence
    : null;
}

function profileStorageKey(profileId) {
  const api = getProfilePersistence();
  if (api && typeof api.profileKey === "function") return api.profileKey(profileId);
  return `LL:profile:${String(profileId || "").trim()}:state`;
}
...
function makeFreshProfileState() {
  return jsonClone(DEFAULT_PROFILE_STATE, {
    prog: { meta: {}, stars: {} },
    mnemoMap: {},
    time: { byDay: {}, lastDay: "", streak: 0 },
    settings: { topic: "all", level: "all", starFilter: "all", tts: "off", speed: 0.9 },
    ui: { mode: "level", uiMode: "normal" },
    daily: { lastDoneDay: "", count: 0 },
    boxFocus: [],
    deckOverrides: { deById: {} },
    glossary: { items: {}, ui: { search: "", type: "all", lang: "all", genus: "all", sort: "newest" } },
    lookup: { mini: {}, phraseMap: {} },
  });
}

function collectState() {
  return {
    schemaVersion: PROFILE_STATE_SCHEMA,
    prog: jsonClone(state.prog, { meta: {}, stars: {} }),
    mnemoMap: jsonClone(state.mnemoMap, {}),
    time: jsonClone(state.time, { byDay: {}, lastDay: "", streak: 0 }),
    settings: jsonClone(state.settings, { topic: "all", level: "all", starFilter: "all", tts: "off", speed: 0.9 }),
    ui: {
      mode: state.ui.mode || "level",
      uiMode: state.ui.uiMode || "normal",
    },
    daily: jsonClone(state.daily, { lastDoneDay: "", count: 0 }),
    boxFocus: jsonClone(state.boxFocus, []),
    deckOverrides: jsonClone(state.deckOverrides, { deById: {} }),
    glossary: {
      items: jsonClone(state.glossary?.items || {}, {}),
      ui: jsonClone(state.glossary?.ui || {}, { search: "", type: "all", lang: "all", genus: "all", sort: "newest" }),
    },
    lookup: {
      mini: jsonClone(state.lookup?.mini || {}, {}),
      phraseMap: jsonClone(state.lookup?.phraseMap || {}, {}),
    },
  };
}

function applyState(nextState, { refreshUi = true } = {}) {
  const fresh = makeFreshProfileState();
  const incoming = nextState && typeof nextState === "object" ? nextState : {};

  state.prog = jsonClone(incoming.prog, fresh.prog);
  if (!state.prog || typeof state.prog !== "object") state.prog = jsonClone(fresh.prog, { meta: {}, stars: {} });
  if (!state.prog.meta || typeof state.prog.meta !== "object") state.prog.meta = {};
  if (!state.prog.stars || typeof state.prog.stars !== "object") state.prog.stars = {};
  for (const m of Object.values(state.prog.meta)) {
    if (!m || typeof m !== "object") continue;
    if (!("box" in m) && ("level" in m)) m.box = m.level;
    m.box = clamp(Number(m.box) || 1, 1, BOX_COUNT);
    m.due = Number(m.due) || 0;
  }

  state.mnemoMap = jsonClone(incoming.mnemoMap, fresh.mnemoMap);

  state.time = { ...jsonClone(fresh.time, fresh.time), ...jsonClone(incoming.time, fresh.time) };
  if (!state.time.byDay || typeof state.time.byDay !== "object") state.time.byDay = {};
  state.time.lastDay = typeof state.time.lastDay === "string" ? state.time.lastDay : "";
  state.time.streak = Math.max(0, Math.floor(Number(state.time.streak) || 0));

  state.settings = { ...jsonClone(fresh.settings, fresh.settings), ...jsonClone(incoming.settings, fresh.settings) };
  state.settings.topic = String(state.settings.topic || "all");
  state.settings.level = String(state.settings.level || "all");
  state.settings.pair = String(state.settings.pair || "all");
  state.settings.dir = String(state.settings.dir || "both");
  state.settings.starFilter = state.settings.starFilter === "starred" ? "starred" : "all";
  state.settings.tts = ["off", "de", "en", "both"].includes(state.settings.tts) ? state.settings.tts : "off";
  state.settings.speed = clamp(Number(state.settings.speed) || 0.9, 0.5, 1.5);

  state.daily = { ...jsonClone(fresh.daily, fresh.daily), ...jsonClone(incoming.daily, fresh.daily) };
  state.daily.lastDoneDay = typeof state.daily.lastDoneDay === "string" ? state.daily.lastDoneDay : "";
  state.daily.count = Math.max(0, Math.floor(Number(state.daily.count) || 0));

  state.boxFocus = normalizeBoxFocusList(incoming.boxFocus ?? fresh.boxFocus);

  state.deckOverrides = { deById: {} };
  const deById = incoming.deckOverrides?.deById || fresh.deckOverrides?.deById || {};
  if (deById && typeof deById === "object") {
    for (const [id, text] of Object.entries(deById)) {
      const k = String(id || "").trim();
      const v = String(text || "").trim();
      if (k && v) state.deckOverrides.deById[k] = v;
    }
  }

  state.glossary.items = {};
  const glossaryItems = incoming.glossary?.items || fresh.glossary.items || {};
  for (const [k, v] of Object.entries(glossaryItems || {})) {
    const norm = normalizeGlossaryEntry({ ...v, key: k });
    if (!norm) continue;
    state.glossary.items[norm.key] = norm;
  }
  state.glossary.ui = {
    ...jsonClone(fresh.glossary.ui, fresh.glossary.ui),
    ...jsonClone(incoming.glossary?.ui, fresh.glossary.ui),
  };
  ...
  state.lookup.mini = jsonClone(incoming.lookup?.mini, fresh.lookup.mini);
  state.lookup.phraseMap = jsonClone(incoming.lookup?.phraseMap, fresh.lookup.phraseMap);

  state.ui.mode = ["daily", "level", "review"].includes(incoming.ui?.mode) ? incoming.ui.mode : fresh.ui.mode;
  state.ui.uiMode = incoming.ui?.uiMode === "background" ? "background" : "normal";

  state.session.currentId = null;
  state.session.currentCard = null;
  state.session.queue = [];
  state.session.timerStart = 0;
  state.session.timerRunning = false;
  state.session.kind = "level";
  state.session.stats = null;
  ...
  syncTtsRuntimeAfterApplyState(incoming);
}

function syncTtsRuntimeAfterApplyState(appliedState) {
  try {
    if (els.tts) els.tts.dispatchEvent(new Event("change", { bubbles: true }));
    if (els.speed) els.speed.dispatchEvent(new Event("input", { bubbles: true }));
    if (els.pair) els.pair.dispatchEvent(new Event("change", { bubbles: true }));
    if ("speechSynthesis" in window) speechSynthesis.cancel();
    syncTtsIntoStateAndPersist("after-applyState");
    console.log(
      "[TTS] applyState profile=",
...
function saveActiveProfileStateNow() {
  const api = getProfilePersistence();
  if (!api || !activeProfileId) return;
  try {
    api.saveProfileState(activeProfileId, collectState());
  } catch (_) {}
}

function updateProfileUi() {
  const api = getProfilePersistence();
  const profiles = api ? api.listProfiles() : [];
  const active = profiles.find((p) => p.id === activeProfileId) || null;
  if (els.profileLabel) {
    els.profileLabel.textContent = `Profil: ${active ? active.name : "—"}`;
    els.profileLabel.title = active ? `Aktiv: ${active.name}` : "Kein aktives Profil";
  }
  if (els.logout) els.logout.disabled = !activeProfileId;
}
...
function promptForProfileId() {
  ...
  while (true) {
    ...
    if (!raw) {
      ...
      const pid = api.createProfile("Gast");
      return pid;
    }
    if (/^n$/i.test(raw)) {
      const name = window.prompt("Name für neues Profil:", "Gast");
      const pid = api.createProfile(String(name || "Gast").trim() || "Gast");
      return pid;
    }
    const idx = Number(raw);
    if (Number.isFinite(idx) && idx >= 1 && idx <= profiles.length) {
      return profiles[idx - 1].id;
    }
    alert("Bitte Zahl aus der Liste oder N eingeben.");
  }
}

function hasAnyLegacyState() {
  const keys = [
    STORAGE_KEY,
    MNEMO_KEY,
    TIME_KEY,
...
function migrateLegacyStorageIfNeeded() {
  ...
  try {
    ...
    applyState(makeFreshProfileState(), { refreshUi: false });
    loadSettings();
    loadBoxFocus();
    ...
    const pid = api.createProfile("Gast");
    api.saveProfileState(pid, collectState());
    api.setActiveProfile(pid);
    activeProfileId = pid;
    localStorage.setItem(LEGACY_MIGRATION_FLAG, "1");
  } catch (_) {}
}

function switchToProfile(profileId, { saveCurrent = true, announce = true } = {}) {
  const api = getProfilePersistence();
  if (!api) return false;
  const pid = String(profileId || "").trim();
  if (!pid) return false;
  if (saveCurrent && activeProfileId) saveActiveProfileStateNow();

  if (!api.setActiveProfile(pid)) return false;
  activeProfileId = pid;
  const loaded = api.loadProfileState(pid);
  applyState(loaded && typeof loaded === "object" ? loaded : makeFreshProfileState());
  ensureDefaultPhraseMapSeeds();
  queueProfileSave();
  updateProfileUi();

  if (announce) {
    const p = api.listProfiles().find((x) => x.id === pid);
    info(`Profil aktiv: ${p?.name || "Profil"}`);
  }
  return true;
}
...
async function ensureProfileBootstrap() {
  ...
  const api = getProfilePersistence();
  if (api && typeof api.debounce === "function") {
    scheduleProfileSave = api.debounce(() => {
      saveActiveProfileStateNow();
    }, 300);
  } else {
    scheduleProfileSave = () => {};
  }
  ...
  let pid = api ? api.getActiveProfile() : null;
  if (!pid && api) pid = promptForProfileId();

  if (api && pid) {
    api.setActiveProfile(pid);
    activeProfileId = pid;
    const loaded = api.loadProfileState(pid);
    applyState(loaded && typeof loaded === "object" ? loaded : makeFreshProfileState(), { refreshUi: false });
    queueProfileSave();
    updateProfileUi();
  } else {
...
function normalizeItem(raw, idx) {
  const from = String(pick(raw, ["_from", "from", "srcLang"]) || "de").trim().toLowerCase() || "de";
  const to = String(pick(raw, ["_to", "to", "dstLang", "targetLang"]) || "en").trim().toLowerCase() || "en";
  ...
  if (isJunkText(front) || isJunkText(back))
  ...
  const derived = `${topic}::${stufe}::${from}::${to}::${front}::${back}`;
  const id = givenId || `${strHash(derived)}::${idx}`;

  const out = { id, topic, stufe, level: stufe, mnemo, _from: from, _to: to };
  out[from] = front;
  out[to] = back;
  // Backward-compatible aliases for existing app logic (search/index/glossary paths still expect de/en)
  out.de = front;
  out.en = back;
  return out;
}
...
function cardFromCode(it) {
  return String(it?._from || "de").trim().toLowerCase() || "de";
}

function cardToCode(it) {
  return String(it?._to || "en").trim().toLowerCase() || "en";
}

function cardFrontText(it) {
  if (!it) return "";
  const from = cardFromCode(it);
  return String(it[from] ?? it.de ?? it.front ?? "").trim();
}

function cardBackText(it) {
  if (!it) return "";
  const to = cardToCode(it);
  return String(it[to] ?? it.en ?? it.back ?? "").trim();
}

function langLabel(code, fallback = "en") {
  return String(code || fallback || "").trim().toUpperCase() || String(fallback || "EN").toUpperCase();
}

function ttsLangForCode(code) {
  ...
  return map[c] || (c ? `${c}-${c.toUpperCase()}` : "");
}

function applyCardLangLabels(it) {
  const from = cardFromCode(it);
  const to = cardToCode(it);
  if (els.frontLangLbl) els.frontLangLbl.textContent = langLabel(from, "de");
  if (els.backLangLbl) els.backLangLbl.textContent = langLabel(to, "en");
}

function applyDeckMetaToRows(data, arr) {
  let rows = Array.isArray(arr) ? arr : [];
  const meta = data && typeof data === "object" ? data.meta : null;
  if (!meta || typeof meta !== "object" || !rows.length) return rows;

  const metaTopic = String(meta.id || "").trim();
  const metaLevel = meta.type === "song" ? "Song" : "";
  const metaFrom = String(meta.from || "").trim().toLowerCase();
  const metaTo = String(meta.to || "").trim().toLowerCase();

  rows = rows.map((row) => ({
    ...(row || {}),
    ...(metaTopic ? {
      topic: String((row && row.topic) || metaTopic).trim() || metaTopic,
      stufe: String((row && (row.stufe || row.level)) || metaLevel || "A1").trim() || "A1",
      level: String((row && (row.level || row.stufe)) || metaLevel || "A1").trim() || "A1",
    } : {}),
    ...(metaFrom ? { _from: String((row && row._from) || metaFrom).trim().toLowerCase() || metaFrom } : {}),
    ...(metaTo ? { _to: String((row && row._to) || metaTo).trim().toLowerCase() || metaTo } : {}),
  }));
  return rows;
}

function normalizeLangCode(code, fallback = "de") {
  const v = String(code || "").trim().toLowerCase();
  return v || String(fallback || "de").toLowerCase();
}

function pairKeyForCodes(a, b) {
  const aa = normalizeLangCode(a, "de");
  const bb = normalizeLangCode(b, "en");
  return [aa, bb].sort().join("|");
}

function pairKeyForCard(it) {
  return pairKeyForCodes(cardFromCode(it), cardToCode(it));
}

function pairSpecFromKey(key) {
  const raw = String(key || "").trim();
  if (!raw || raw === "all") return null;
  const [a, b] = raw.split("|");
  if (!a || !b) return null;
  return { from: normalizeLangCode(a, "de"), to: normalizeLangCode(b, "en"), key: `${normalizeLangCode(a)}|${normalizeLangCode(b)}` };
}

function pairLabelFromKey(key) {
  const spec = pairSpecFromKey(key);
  if (!spec) return "Alle";
  return `${langLabel(spec.from, "de")}↔${langLabel(spec.to, "en")}`;
}

function getAvailablePairs(cards) {
  const set = new Set();
  for (const it of cards || []) {
    if (!isUsableCard(it)) continue;
    set.add(pairKeyForCard(it));
  }
  return Array.from(set).sort((a, b) => pairLabelFromKey(a).localeCompare(pairLabelFromKey(b)));
}

function fillPairSelect() {
  if (!els.pair) return;
  const pairs = getAvailablePairs(state.all);
  els.pair.innerHTML = "";
  const allOpt = document.createElement("option");
  allOpt.value = "all";
  allOpt.textContent = "Alle";
  els.pair.appendChild(allOpt);
  for (const key of pairs) {
    const opt = document.createElement("option");
    opt.value = key;
    opt.textContent = pairLabelFromKey(key);
    els.pair.appendChild(opt);
  }
  const want = String(state.settings.pair || "all");
  els.pair.value = [...els.pair.options].some((o) => o.value === want) ? want : "all";
  state.settings.pair = els.pair.value || "all";
}

function updateDirectionSelect() {
  if (!els.dir) return;
  const pair = pairSpecFromKey(state.settings.pair);
  els.dir.innerHTML = "";
  ...
  if (!pair) {
    els.dir.appendChild(mk("both", "↔"));
  } else {
    els.dir.appendChild(mk("both", "↔"));
    els.dir.appendChild(mk("fwd", `${langLabel(pair.from)}→${langLabel(pair.to)}`));
    els.dir.appendChild(mk("rev", `${langLabel(pair.to)}→${langLabel(pair.from)}`));
  }
  const want = String(state.settings.dir || "both");
  els.dir.value = [...els.dir.options].some((o) => o.value === want) ? want : "both";
  state.settings.dir = els.dir.value || "both";
}

function syncPairDirectionControls() {
  fillPairSelect();
  updateDirectionSelect();
}

function cloneCardWithDirectionSwap(it) {
  if (!it) return it;
  const front = cardFrontText(it);
  const back = cardBackText(it);
  const from = cardFromCode(it);
  const to = cardToCode(it);
  const clone = { ...it };
  clone._from = to;
  ...
  clone.de = back;
  clone.en = front;
  clone._viewSwapped = true;
  return clone;
}

function alignCardToDirection(it, wantFrom, wantTo) {
  const f = cardFromCode(it);
  const t = cardToCode(it);
  if (f === wantFrom && t === wantTo) return it;
  if (f === wantTo && t === wantFrom) return cloneCardWithDirectionSwap(it);
  return null;
}

function getDirectionalSessionCards(items) {
  const pair = pairSpecFromKey(state.settings.pair);
  const dir = String(state.settings.dir || "both");
  if (!pair) return Array.isArray(items) ? items : [];

  const out = [];
  for (const it of items || []) {
    if (!it) continue;
    if (pairKeyForCard(it) !== pair.key) continue;
    if (dir === "both") {
      out.push(it);
      continue;
    }
    if (dir === "fwd") {
      const v = alignCardToDirection(it, pair.from, pair.to);
      if (v) out.push(v);
      continue;
    }
    if (dir === "rev") {
      const v = alignCardToDirection(it, pair.to, pair.from);
      if (v) out.push(v);
      continue;
    }
    out.push(it);
...
function getCurrentSessionCard() {
  if (state.session.currentCard && state.session.currentCard.id === state.session.currentId) {
    return state.session.currentCard;
  }
  const id = state.session.currentId;
  if (!id) return null;
  return state.all.find((x) => x.id === id) || null;
}

function normalizeTtsMode(x) {
  const v = String(x ?? "").trim().toLowerCase();
  if (["off", "0", "false", "aus"].includes(v)) return "off";
  if (["front", "vorn", "1", "de", "deutsch"].includes(v)) return "front";
  if (["back", "hinten", "2", "en", "english"].includes(v)) return "back";
  if (["both", "beide", "3", "on", "true"].includes(v)) return "both";
  return "both";
}

function getUiTtsMode() {
  // Immer vorlesen: selbst wenn UI "off" steht, erzwingen wir "both"
  const m = normalizeTtsMode(els.tts?.value);
  return m === "off" ? "both" : m;
}

function syncTtsIntoStateAndPersist(reason) {
  const m = getUiTtsMode();
  state.settings = state.settings || {};
  state.settings.ttsMode = m;
  state.settings.tts = m === "front" ? "de" : (m === "back" ? "en" : m);
  state.ttsMode = m;

  const r = Number(els.speed?.value);
  const rate = Number.isFinite(r) && r > 0 ? r : 1;
  state.settings.ttsRate = rate;
  state.settings.speed = rate;
  state.ttsRate = rate;

  try { saveSettings(); } catch (_) {}
  try { queueProfileSave(); } catch (_) {}
...
function speakFrontIfEnabled(it) {
  if (!it) return;
  const mode = getUiTtsMode();
  if (mode === "off") return;
  if (!["front", "both"].includes(mode)) return;
  speak(cardFrontText(it), ttsLangForCode(cardFromCode(it)));
}

function speakCurrentSideIfEnabled(it, side) {
  if (!it) return;
  const s = side === "back" ? "back" : "front";
  const mode = getUiTtsMode();
  if (mode === "off") return;
  if (s === "front" && (mode === "front" || mode === "both")) {
    speak(cardFrontText(it), ttsLangForCode(cardFromCode(it)));
  } else if (s === "back" && (mode === "back" || mode === "both")) {
    speak(cardBackText(it), ttsLangForCode(cardToCode(it)));
  }
}
...
function fillSelect(sel, values, current) {
  ...
  for (const v of values) {
    const opt = document.createElement("option");
    opt.value = v;
    const isTopicSelect = sel === els.topic;
    opt.textContent = v === "all" ? "Alle" : (isTopicSelect ? (TOPIC_LABELS[v] || v) : v);
    sel.appendChild(opt);
  }
...
function buildQueue({ onlyBox = null, focusBoxes = null, dueOnly = false, limit = null, allowTrainingFallback = false } = {}) {
  ...
  const sessionPool = getDirectionalSessionCards(state.filtered);

  const due = [];
  const fresh = [];
  const trainingPool = [];

  for (const it of sessionPool) {
    if (!isUsableCard(it)) continue;
...
function nextCard() {
  ...
  if (els.front) els.front.textContent = cardFrontText(it);
  if (els.back) els.back.textContent = cardBackText(it);
  // Test-Hinweis: Song-Deck mit meta.to="ru" zeigt hier "RU" statt "EN".
  applyCardLangLabels(it);
...
function grade(known) {
  ...
  const id = state.session.currentId;
  const it = getCurrentSessionCard();
  if (!it) return;
  ...
  if (revealedNow || isAnswerVisible()) speakCurrentSideIfEnabled(it, "back");
...
function onFlip() {
  ...
  speakCurrentSideIfEnabled(it, flipped ? "back" : "front");
}

function resetAll() {
  if (!confirm("Aktives Profil wirklich zurücksetzen? Alle Daten dieses Profils werden gelöscht.")) return;
  const pid = activeProfileId;
  if (!pid) {
    info("Kein aktives Profil.");
    return;
  }
  try {
    localStorage.removeItem(profileStorageKey(pid));
  } catch (_) {}
  applyState(makeFreshProfileState());
  ensureDefaultPhraseMapSeeds();
  queueProfileSave();
  info("Aktives Profil zurückgesetzt.");
}
...
function syncControls() {
  ...
  if (els.pair) els.pair.value = state.settings.pair || "all";
  if (els.dir) els.dir.value = state.settings.dir || "both";
...
function applySpeedPreset(value) {
  ...
  saveSettings();
  writeHash();
  syncTtsIntoStateAndPersist("preset");
}
...
function clampSettingsToData() {
  ...
  const okPairs = new Set(["all", ...getAvailablePairs(state.all)]);
  ...
  if (!okPairs.has(state.settings.pair)) state.settings.pair = "all";
  if (!["both", "fwd", "rev"].includes(state.settings.dir)) state.settings.dir = "both";
...
// ---------- TTS ----------
function normalizeTtsLangTag(langTag) {
  ...
  if (/^[a-z]{2,3}$/i.test(raw)) return map[lower] || raw;
  return raw;
}
...
function chooseVoice(langTag) {
  ...
  const wanted = normalizeTtsLangTag(langTag).toLowerCase();
...
function safeSpeak(text, langTag, rate) {
  if (!("speechSynthesis" in window)) return;
  const clean = String(text || "").trim();
  if (!clean) return;

  const u = new SpeechSynthesisUtterance(clean);
  u.lang = normalizeTtsLangTag(langTag);
  u.rate = clamp(Number(rate) || Number(state.settings.speed) || 1, 0.5, 1.5);
  const v = chooseVoice(u.lang);
  if (v) {
    u.voice = v;
  } else {
    console.warn("[TTS] No voice for", u.lang, "-> fallback to default voice");
  }
...
function speak(text, lang) {
  safeSpeak(text, lang, state.settings.speed);
}
...
async function loadOptionalDecks(urls) {
  ...
  for (const url of urls || []) {
    try {
      ...
      arr = applyDeckMetaToRows(data, arr);
...
async function loadDeck() {
  ...
  const arr = applyDeckMetaToRows(data, extractArray(data));
...
async function init() {
  ...
  on(els.tts, "change", () => {
    ...
    saveSettings();
    writeHash();
    syncTtsIntoStateAndPersist("ui-change");
  });
  on(els.speed, "input", () => {
  ...
  on(els.logout, "click", () => {
    const api = getProfilePersistence();
    if (!api) {
      info("Profil-System nicht verfügbar.");
      return;
    }
    saveActiveProfileStateNow();
    api.logoutProfile();
    activeProfileId = null;
    updateProfileUi();
    const pid = promptForProfileId();
    if (!pid) return;
    switchToProfile(pid, { saveCurrent: false, announce: true });
  });
  ...
  on(els.speak, "click", () => {
    ...
    if (flipped) speak(cardBackText(it), ttsLangForCode(cardToCode(it)));
    else speak(cardFrontText(it), ttsLangForCode(cardFromCode(it)));
  });
